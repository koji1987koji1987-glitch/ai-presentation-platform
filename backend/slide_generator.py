...
def generate_slides(text, user_prompt=None, slide_count=None):
    ...
    prompt = (
        ...
        "1. DISTRIBUTE CONTENT & AVOID BLANK SLIDES: You must distribute the source material or topic details evenly across the requested slide count. Do NOT write generic filler text or leave the last few slides blank or with empty content arrays. Every single slide must contain high-quality, substantial, and specific content points.\n"
        "2. MIXTURE OF TEXT & GRAPHICS: Ensure a rich balance of text-only lists and graphics/diagrams. At least 40% of the slides in the deck MUST use structured graphical layouts: 'timeline' (for roadmaps, processes, chronologies), 'comparison' (for tables, matrix evaluations), 'cards' (for core pillars/focus areas), 'process_flow' (for step-by-step sequences), or 'statistics' (for charts and key numbers). Do NOT default to 'bullet_list' or 'two_column' for all slides.\n"
        "3. NO TRUNCATION OR ELLIPSES: All headers, titles, subtitles, and bullet points must be fully formed, complete sentences or phrases. Never end a point with '...' or leave it cut off. Write complete details.\n\n"
